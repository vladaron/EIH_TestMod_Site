...
var_new_names <- c("age" = "Age (years)" ,"bmi" = "BMI (kg/m^2^)" ,"FlANDERS_lat"= "FLANDERS (n)" , "level_education" = "Level of education (n)" , "W_peak_rel" = "Relative PPO (W/kg)", "HR_max" = "Maximal HR (bpm)" , "STAI_2"= "STAI-Y2 (/80)" , "PCS" = "PCS total (/51)" , "FPQ" = "FPQ-15 (/100)" , "IPAQ_category" = "IPAQ category (n)", "IPAQ_total_moderate"= "IPAQ total mPA (MET/min/week)^a^" , "IPAQ_total_vigourous" = "IPAQ total vPA (MET/min/week)^a^" , "IPAQ_total_pa"= "IPAQ total PA (MET/min/week)^a^", "W_peak" = "PPO (W)", "TSK" = "TSK (/37)" )
...
ipaq_cat <- as_tibble(table(subset(data, session == "session_1", IPAQ_category))) |> 
  mutate(IPAQ_category = factor(IPAQ_category, levels = c("LOW", "MODERATE", "HIGH"), labels = c("Low", "Moderate", "High"))) |> 
  rename(levels = IPAQ_category) |>  
  rename(value = n) 
ipaq_cat$var <- c(rep("IPAQ_category",nrow(ipaq_cat)))

flanders <- as_tibble(table(subset(data, session == "session_1", FlANDERS_lat))) |> 
  rename(levels = FlANDERS_lat) |>  
  rename(value = n) 
flanders$var <- c(rep("FlANDERS_lat",nrow(flanders)))

educ <- as_tibble(table(subset(data, session == "session_1", level_education))) |> 
...
p_delta_temp_points <- delta_temp %>% 
      ...
      arrange(unique_ID) %>% 
      ggplot() +
      geom_point(aes(x = site, y = abc_temp, colour = condition ), size = 1, position = position_jitter(width = 0.05, seed = 123)) +
...
legend.position = c(0.6, 0.9), legend.direction = "horizontal"))
...
p_mean_temp <-as.ggplot( pt1) /as.ggplot( pt2)/ as.ggplot(pt3)/ as.ggplot(pt4) + plot_layout(axis_titles = "collect") 
...
l <- as.ggplot(pt1)/as.ggplot(pt2)/as.ggplot(pt3)/as.ggplot(pt4) + plot_layout(ncol = 1)
r <- p_delta_temp_points / p_delta_temp_mean 

ptemp_all <- (l|r )+ plot_layout(ncol = 2, widths = c(1,2))+ plot_annotation(tag_levels = 'A' ) & theme(
...
m <- lmer(eih ~  test_mod*site + (1|ID), data = dm )
emm <- emmeans(m, ~ test_mod*site)
pairs(emm, simple = "test_mod", adjust = "holm")
```
