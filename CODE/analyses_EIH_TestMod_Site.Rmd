...
rel_pre <- function(data, anov = T) {
 ...
 if (anov == T) {
  ...
  icc_df <- rbind(
    icc.res.qppt$results |> filter(type == "ICC3"),
    icc.res.qcdt$results |> filter(type == "ICC3"),
    icc.res.qhpt$results |> filter(type == "ICC3"),
    icc.res.qpp$results |> filter(type == "ICC3"),
    
    icc.res.fppt$results |> filter(type == "ICC3"),
    icc.res.fcdt$results |> filter(type == "ICC3"),
    icc.res.fhpt$results |> filter(type == "ICC3"),
    icc.res.fpp$results |> filter(type == "ICC3"),
    
    icc.res.adt$results |> filter(type == "ICC3")
  )
  ...
  t.icc <- icc_df |>
    ...
    kbl(caption = paste("ICC values (", comput, ") at all sites & test modalities"),
        col.names = c(site = "Site", test_mod = "Test modality", ICC = "ICC(3C,1)", `lower bound` = "CI low", `upper bound` = "CI high")) |>
    kable_minimal() |> 
...
# ADT
padt <- (plot_prepost_an(dm.adt, "Auditory detection threshold", "Arbitrary units [0,1]", df_an = an_mpp, v = "ADT_No site"))
...
ggsave("/Users/vladimiraron/Library/CloudStorage/OneDrive-UCL/PhD/Projects/WP1/writing/figures_wp1/fig5_mpp.png", plot = p_mpp, dpi = 600, height = 9, width = 10)
