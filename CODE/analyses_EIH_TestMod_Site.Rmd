...
cooks_outliers <- function(lmm, d, criteria = 1){
  ...
  if(is_empty(influential)){
    cat("No influential observation detected")
    
  } else {
    cat(length(influential), "Influential observation(s) detected. Observation(s) number: ", influential)
...
check_mod <-function(lmm,data){
  ...
  pfit <- ggplot(data = {{data}}, aes(x = predicted, y = resid)) +
    ...
    my_theme()+
    labs(title = paste("Fitted values ~ Residuals; ", mod.info))  
...
## Data baseline
data_b <- read_excel(here("input","crf_eih.xlsx"), na = c("NM","missing"), sheet = "baseline", skip = 1, col_names = TRUE) %>% select(!dominance)
data_b <- data_b[1:n,]

## data test session
data_t <- read_excel(here("input","crf_eih.xlsx"), na = c("NM","missing"), sheet = "test_data", skip = 0, col_names = TRUE) 
data_t <- data_t[1:n,]
...
# data sessions
data_1 <- read_excel(here("input", "crf_eih.xlsx"), na = c("NM","missing"), sheet = "session_1", skip = 2, col_names = TRUE)  # import data from session 1
data_1 <- data_1[1:n,] |> select(-time_pos)


data_2 <- read_excel(here("input", "crf_eih.xlsx"), na = c("NM","missing"), sheet = "session_2", skip = 2, col_names = TRUE)  # import data from session 2
data_2 <- data_2[1:n,]
...
# removing values ADT post for subject 10  --> technical issue 
...
data_long <- full_join(d_tms_long, d_mean_temp_long) |> # data set for skin temperature and tms are merged
  ...
  mutate(test_value = if_else(test_mod == "ppt", test_value*10, test_value)) # converting N/cm2 to kilospascale
...
Summary statistics of participants data and scores to questionnaires are computed and formatted in a table.

```{r ts1}


var_new_names <- c("age" = "Age (years)" ,"bmi" = "BMI (kg/m^2^)" ,"FlANDERS_lat"= "FLANDERS (n)" , "level_education" = "Level of education (n)" , "W_peak_rel" = "Relative PPO (W/kg)", "HR_max" = "Maximal HR (bpm)" , "STAI_2"= "STAI-Y2 (/80)" , "PCS" = "PCS total (/52)" , "IPAQ_category" = "IPAQ category (n)", "IPAQ_total_moderate"= "IPAQ total mPA (MET/min/wk)" , "IPAQ_total_vigourous" = "IPAQ total vPA (MET/min/wk)" , "IPAQ_total_pa"= "IPAQ total PA (MET/min/wk)", "W_peak" = "PPO (W)", "TSK" = "TSK (/68)" )


cont_data <- var_summary(data, var_vec = c(age, bmi, W_peak, W_peak_rel, HR_max, STAI_2, PCS, TSK, IPAQ_total_moderate, IPAQ_total_vigourous, IPAQ_total_pa)) |> 
  mutate(mean = if_else(var == "HR_max",specify_decimal(mean, 0), specify_decimal(mean, 1) )) |> 
  mutate(sd = if_else(var == "HR_max",specify_decimal(sd, 0), specify_decimal(sd, 1) )) |> 
  mutate(median = if_else(var %in% c("IPAQ_total_pa", "IPAQ_total_moderate", "IPAQ_total_vigourous"),specify_decimal(median, 0), specify_decimal(median, 1) )) |> 
  mutate(IQR = if_else(var %in% c("IPAQ_total_pa", "IPAQ_total_moderate", "IPAQ_total_vigourous"),specify_decimal(IQR, 0), specify_decimal(IQR, 1) )) |> 
    mutate(across(where(is.numeric), specify_decimal,1)) |> 
...
ipaq_cat <- as_tibble(table(subset(data, condition == "exp", IPAQ_category))) |>
  ...
  rename(value = n) |> 
  mutate(levels = str_to_title(levels))
...
order_rows <- c("age", "bmi", "FLANDERS_lat", "level_education", "W_peak", "W_peak_rel", "HR_max", "STAI_2", "PCS", "TSK", "IPAQ_total_moderate", "IPAQ_total_vigourous", "IPAQ_total_pa", "IPAQ_category")
...
t1 <- rbind(cont_data, flanders, ipaq_cat) |> 
  ...
  colformat_md(part = "body", j = 1) |> 
    hline(i = c(1:11,14), border = dash_border)  |> 
  add_footer_lines(values = c("Data presented as mean ± SD or a as median [IQR]. Units or maximum scores are indicated in parentheses. \n Abbreviations: BMI, Body mass Index; PPO, Peak power output; HR, Heart rate; STAI-Y2, State-trait anxiety inventory (form 2, trait); PCS, Pain catastrophizing scale; TSK, Tampa scale for kinesiophobia; IPAQ, International physical activity questionnaire; FLANDERS, Flinders handedness survey; kg, kilogram; W, watt; bpm, beats per minute; MET, Metabolic equivalent;min, minute; wk, week; v/mPA, vigorous/moderate physical activity. "
  )) %>% 
  valign(part = "body", valign = "top") |> 
  align(j = 1:2, part = "body", align = "left") |> 
  align(j = 3, part = "body", align = "right") |> 
...
# diff days between sessions
condition_days <- data |> select(session, condition, ID, time_to_target, date, time_position) |> 
  select(-session, ID, condition, date, -time_position, -time_to_target) |> 
  mutate(date= as.Date(as.numeric(date), origin = "1899-12-30")) |> 
  pivot_wider(names_from = condition, values_from = date) |> 
  mutate(diff_days = abs(as.numeric(difftime(exp, cont, units = "days")))) |> 
  arrange(diff_days) |>
  pull(diff_days) |> 
  summary(diff_days)
...
time_to_hrr <- data |> select(ID, condition, date, time_to_target) |> 
  filter(condition == "exp") |> 
  mutate(time_sec = as.numeric(time_to_target) * 86400) |>
  mutate(min = hour(time_sec),
         sec = minute(time_sec)) |> 
  mutate(sec_total = 60*min + sec) |> 
  summarise(msec = mean(sec_total, na.rm = T), sd_sec = sd(sec_total, na.rm = T)) |>  mutate(
    msec_minsec = sprintf("%02d:%02d", floor(msec/60), round(msec %% 60)),
    sd_minsec   = sprintf("%02d:%02d", floor(sd_sec/60), round(sd_sec %% 60))
  ) |>  mutate(msd = glue("{msec_minsec} ± {sd_minsec}")) |> 
  pull(msd)
...
cat(
  "Days between conditions \n")
print(condition_days)

cat(
  "Mean ± SD time to reach target % of HRR (min:sec) \n")
print(time_to_hrr)
...
The random effects follow a normal distribution with fat tails, relatively symmetrical. No heteroskedasticity is identified. Residuals do not follow normality in the post:exp factor. Cooks distance < 1 suggest little influence of deviated data point. 
...
Normal effects appear slighlty deviated from normality. No heteroskedasticity is identified. Residuals do not follow normality in the post:exp factor. Cooks distance < 1 suggest little influence of deviated data point. 
...
Random effects are normally distributed. No heteroskedasticity is identified and residuals appear normally distributed across factors. No influential observation is detected.
...
Random effects are normally distributed. No heteroskedasticity is identified. One data point was found to display a cook's distance > 1 and was removed. Slight deviations from normality appear pre:exp and pre:cont factors. No other influential observation is detected. The model is estimated. 
...
Room humidity and room temperature values appear normally distributed. Scores of RPE, SSS, and STAI-Y1 seem to violate normality assumptions. 
...
df_t2 <- full_join(d.covar.sum,d.cont.simple.sum ) |> 
  mutate(across(c(m, sd, md, iqr),specify_decimal,0 )) |>
  mutate(
    msd = glue("{m} ± {sd}"),
    med_iqr = glue("{md} [{iqr}]")) |> 
  select(-m, -sd, -md, -iqr) |> 
...
t2 <- df_t2 |> 
  ...
  align(j = 2:7, part = "body", align = "right") |> 
   align(part = "header", align = "center") |> 
  add_footer_lines(values = c("Data from 38 participants presented as mean ± SD or median [IQR]. Units or maximum scores are indicated in parentheses. \n Abbreviations: HR, Heart rate; BR, Breating rate; SBP, Systolic blood pressure; DBP, Diastolic blood pressure; BPM, Beats per minute; CPM, Cycles per minute; mmHg, millimeter of mercury; RPE, Rate of perceived exertion; °C, Celsius degrees;  Df, Degrees of freedom; STAI-Y1, State-trait anxiety inventory (form 1, state); SSS, Stanford sleepiness scale.")) 
t2
...
All levels site:condition appear normally distributed except for dominant_quadriceps:control where negative skew is observed, potentially due to an influential observation. The observation is kept. 
...
We estimate the impact of the factors 'site' and 'condition' on the post - pre difference in skin temperature while controlling for pre condition values.
...
Random effects slightly deviate from normality. No heteroskedasticity is identified and residuals slightly deviate from normality (negative skew) in factors Nondominantforearm:Exercise. Cooks distance < 1 suggest little influence of deviated data point.
...
my_theme_temp <- function(base_size = 11, 
                     base_family = 'Roboto',
                     base_line_size = base_size / 22,
                     base_rect_size = base_size / 22,
                     md = T, col_x = "black", col_y = "black") {
  theme(
    #plot.margin = margin(t=1,r=1,b=1,l=1, unit = "cm"),
    plot.margin = margin(0.5, 0.5, 1, 0.5),
    axis.line.y = element_line(colour = "black"), 
    axis.line.x = element_line(colour = "black"), 
    panel.background = element_blank(), 
    plot.title = element_text(hjust = 0, size = base_size, face = "bold", colour = "black"), 
    plot.subtitle = element_text(hjust = 0, size = base_size-1,  colour = "black"), 
    axis.title.y = if (md) element_markdown(size = base_size, colour = col_y) else element_text(size = base_size, colour = col_y),
    axis.title.x = element_markdown(size = base_size,angle = 0, hjust = 0.5, colour = col_x) ,
...
grob <- grobTree(textGrob(label, x = 0.1, y = 0.98, hjust=0,
                            gp=gpar(col="black", fontsize=9, fontfamily = "Roboto")))



 y_min <- floor(min(delta_temp$abc_temp) * 1000) / 1000
 y_max <- ceiling(max(delta_temp$abc_temp) * 1000) / 1000
 y_breaks <- pretty(c(y_min*0.98, y_max*1.02))
...
p_delta_temp_points <- delta_temp %>% 
  ...
  scale_y_continuous( limits = y_lim ,  breaks = y_breaks,labels = function(x) format(round(x, 2), nsmall = 1), ) +
      labs(title = "Absolute change of skin temperature",
           subtitle = "Individual values",
        y = "Tskin<sub>abs</sub> (°C)",
        x = "",
        colour = "Condition"
      )  +
    my_theme_temp() +
  theme(legend.position = "none",
    ...
    axis.line.x = element_blank()
        ) +coord_cartesian(ylim = y_lim , xlim = c(0.5, 4.5), expand = F)
...
p_delta_temp_points <- as.ggplot(q)
  ...
  y_min <- floor(min(delta_temp_sum$cil) * 1000) / 1000
 y_max <- ceiling((max(delta_temp_sum$cih) ) * 1000) / 1000
 y_breaks <- pretty(c(y_min*0.98, y_max*1.05))
 y_lim <- c(min(y_breaks), max(y_breaks))
...
p_delta_temp_mean <- delta_temp_sum |> 
  mutate(site = factor(site, levels = c(
    ...
    "<span style='color:black'>Dominant<br>forearm</span>",
    "<span style='color:#925E9FFF'>Non-dominant<br>forearm</span>",
    "<span style='color:#42B540FF'>Dominant<br>quadriceps</span>",
    "<span style='color:black'>Non-dominant<br>quadriceps</span>"
  ))) |> 
  ...
  showSignificance( 2.3, c(0.21, 0.86), "*", width = -0.05) +
    showSignificance( 3.3, c(-0.14, 1), "*", width = -0.05) +
     showSignificance( 4.3, c(-0.26, 1.4), "*", width = -0.05) +
    showSignificance( c(1,2.1), 1.2,"*", width = -0.05, segmentParams = list(col = "#ED0000FF"), textParams = list(col = "#ED0000FF")) +
  showSignificance( c(1,3.1), 1.4,"*", width = -0.05, segmentParams = list(col = "#ED0000FF"), textParams = list(col = "#ED0000FF"))+
  showSignificance( c(1,4.1), 1.8,"*", width = -0.05, segmentParams = list(col = "#ED0000FF"), textParams = list(col = "#ED0000FF")) +
    showSignificance( c(0.9,2), -0.8,"*", width = +0.05, segmentParams = list(col = "#00468BFF"), textParams = list(col = "#00468BFF")) +
    showSignificance( c(1.9,3), -0.4,"*", width = +0.05, segmentParams = list(col = "#00468BFF"), textParams = list(col = "#00468BFF"))+
    showSignificance( c(1.9,4), -0.6,"*", width = +0.05, segmentParams = list(col = "#00468BFF"), textParams = list(col = "#00468BFF")) +
   scale_y_continuous( limits = y_lim ,  breaks = y_breaks,labels = function(x) format(round(x, 2), nsmall = 1), ) +
    coord_cartesian(ylim = y_lim , xlim = c(0.5, 4.5), expand = F)+
...
y_min <- floor(min(d_temp_sum$cil) * 1000) / 1000
y_max <- ceiling(max(d_temp_sum$cih) * 1000) / 1000
y_breaks <- pretty(c(y_min * 0.99, y_max * 1.01))
y_lim <- c(min(y_breaks), max(y_breaks))
...
pos_temp <- position_dodge(width=0.2)
  p <- ds |> 
    mutate(timing = factor(timing, levels = c("Pre", "Post"), labels = c("Pre<br>", "Post<br>"))) |> 
  ggplot(aes(x = timing, y = m, col = condition, group = condition)) +
  geom_point(size = 3, position = pos_temp) +
  ...
  scale_color_manual(values = c("#00468BFF", "#ED0000FF")) +
       scale_y_continuous(  breaks = y_breaks,labels = function(x) format(round(x, 2), nsmall = 1), ) +
    coord_cartesian(ylim = y_lim , xlim = c(0.5, 2.5), expand = F)+
    theme(legend.position = "none") +
  my_theme_temp()  +
  theme(axis.text.x = element_markdown())
   # p
...
pt1 <- (l_temp_plots[[1]]+ scale_x_discrete(breaks = "") + labs(title = "Skin temperature") + guides(col = guide_legend(override.aes = list(size = 3))) + theme(
legend.position = c(0.55, 0.9), legend.direction = "horizontal"))
...
p_mean_temp <- as.ggplot(pt1 /pt2/pt3/pt4+  plot_layout(ncol = 1) + plot_annotation(tag_levels = 'A' )& theme(
  plot.tag = element_text(size = 11, family  = "Roboto", face = "bold")))

l <- p_mean_temp


r <- as.ggplot(p_delta_temp_points / p_delta_temp_mean + plot_annotation(tag_levels = list(c("E", "F")) ) & theme(
  plot.tag = element_text(size = 11, family  = "Roboto", face = "bold") ))

ptemp_all <- (l|r )+ plot_layout(ncol = 2, widths = c(1,2))
...
ggsave("/Users/vladimiraron/Library/CloudStorage/OneDrive-UCL/PhD/Projects/WP1/writing/figures_wp1/fig2_temp.png", plot = ptemp_all, height = 22, width = 19,units = "cm",dpi = 600)

ggsave("/Users/vladimiraron/Library/CloudStorage/OneDrive-UCL/PhD/Projects/WP1/writing/figures_wp1/fig2.tiff", plot = ptemp_all, height = 22, width = 19,units = "cm",dpi = 600)
ggsave("/Users/vladimiraron/Library/CloudStorage/OneDrive-UCL/PhD/Projects/WP1/writing/wp1_manuscript_20251122_ejp/wp1_submission_EJP/fig2.png", plot = ptemp_all, height = 22, width = 19,units = "cm",dpi = 600)
...
Random effects are normally distributed. Overall, no heteroskedasticity is identified and residuals are normally distributed. A slight deviation from normality for forearm:pp residuals. No influential observation is detected. 
...
We compute simple pairwise comparisons of the significant test modality:site interaction with holm correction. 
...
my_theme_modall <- function(base_size = 11, 
                     base_family = 'Roboto',
                     base_line_size = base_size / 22,
                     base_rect_size = base_size / 22,
                     md = T, col_x = "black", col_y = "black") {
...
txt_gain <- text_grob(label = "Increased sensitivity", size = 9,rot = 90, color = "black")
txt_loss <- text_grob(label = "Reduced sensitivity", size = 9,rot = 90, color = "black")

df_label <- anova(m0, ddf = "Kenward-Roger")  |> 
    tidy() |> 
  rowwise() |> 
 mutate(p.value = pvalue_format_plot(p.value))
...
dm_point <- dm %>% 
 ...
 y_min <- floor(min(dm_point$eih) * 1000) / 1000
 y_max <- ceiling(max(dm_point$eih) * 1000) / 1000
 y_breaks <- pretty(c(y_min, y_max*1.2))
 y_lim <- c(min(y_breaks), max(y_breaks))

p.mod.all.points <- dm_point |> 
      ...
      labs(
        y = "Individual mean EIH (z-scores)",
        x = "",
        colour = "Site"
      )  +
    my_theme_modall() +
  theme(legend.position = "none",
    axis.text.x=element_blank(),
        axis.ticks.x=element_blank(),
    axis.line.x = element_blank()
        ) + coord_cartesian(ylim = y_lim, expand = F, xlim = c(0.5, 4.5)) +
    scale_y_continuous( limits = y_lim ,  breaks = y_breaks,labels = function(x) format(round(x, 2), nsmall = 1), expand = c(0,0)) +
  guides(colour = guide_legend(override.aes = list(size = 4))) 
...
p.mod.all.points <- as.ggplot(q)
 ...
 y_min <- floor(min(data.mod.summary$cil) * 1000) / 1000
 y_max <- ceiling(max(data.mod.summary$cih) * 1000) / 1000
 y_breaks <- pretty(c(y_min*1.2, y_max*1.2))
 y_lim <- c(min(y_breaks), max(y_breaks))
...
p.mod.all.means <- ggplot() +
  geom_hline(yintercept = 0, linetype = "dashed", colour = "black") + 
      ...
      geom_point(data = data.mod.summary, aes(x = test_mod, y = m, colour = site, group = site),  position=position_dodge(width=0.2), size = 3) +
       scale_color_manual(values = c("#925E9FFF", "#42B540FF")) +
      labs(
        y = "Condition mean EIH (z-scores)",
        x = "Test modality",
        colour = "Site"
      )  +
    my_theme_modall() +
  ...
  annotation_custom(grob)+
   coord_cartesian(ylim = y_lim, expand = F, xlim = c(0.5, 4.5)) +
    scale_y_continuous( limits = y_lim ,  breaks = y_breaks,labels = function(x) format(round(x, 2), nsmall = 1), expand = c(0,0)) +
     showSignificance( 0.8,c(-0.02, 0.34) , "*", width = +0.05) +
     showSignificance( 4.2, c(-0.39, 0.04), "*", width = -0.05) +
     showSignificance( c(1,3.1), 0.55,"*", width = -0.05,textParams = list(col = "#42B540FF"), segmentParams = list(col = "#42B540FF") ) +
     showSignificance( c(1,4.1), 0.7,"*", width = -0.05,textParams = list(col = "#42B540FF"), segmentParams = list(col = "#42B540FF")) 
...
ggsave("/Users/vladimiraron/Library/CloudStorage/OneDrive-UCL/PhD/Projects/WP1/writing/figures_wp1/fig3_modall.png", plot = p.mod.all, height = 21, width = 18,dpi = 600, units = "cm")

ggsave("/Users/vladimiraron/Library/CloudStorage/OneDrive-UCL/PhD/Projects/WP1/writing/figures_wp1/fig3.tiff", plot = p.mod.all, height = 21, width = 18,dpi = 600, units = "cm")


ggsave("/Users/vladimiraron/Library/CloudStorage/OneDrive-UCL/PhD/Projects/WP1/writing/wp1_manuscript_20251122_ejp/wp1_submission_EJP/fig3.png", plot = p.mod.all, height = 21, width = 18,dpi = 600, units = "cm")
p.mod.all
```
...
## Model absolute change control condition ~ site x test mod

We compute an additional analyses to ensure that differences between test modalities are not only explained by a difference in the control condition. 
...
dm_cont <- data_long %>%
  ...
  pivot_wider(names_from = c(condition, timing), values_from = c(temp, test_value)) |>
  mutate(
    abs_cont =  (test_value_cont_post - test_value_cont_pre),
    delta_temp_cont = (temp_cont_post - temp_cont_pre)
  ) |> 
  select(ID, test_mod, site, order,  delta_temp_cont, abs_cont, test_value_cont_pre) |> 
  mutate(test_mod = factor(test_mod, levels = c("ppt", "pp","hpt", "cdt"), labels = c("PPT", "PP", "HPT", "CDT")),
           site = factor(site, levels = c("forearm", "quad"), labels = c("Forearm", "Quadriceps"))) 
...
mcont <- lmer(abs_cont ~ test_value_cont_pre +   test_mod*site + (1|ID), data = dm_cont)


check_mod(mcont, dm_cont) 

```

Random effects are not perfectly normal. No heteroskedasticity is identified and residuals appear overall normally distributed despite slight deviations for HPT (forearm and quad, negative skew), quadriceps PP (slight negative skew), quadriceps PPT (fat tails). Given all cook values < 1, model is estimated as is. 

```{r mcont_estim}
print(anova(mcont, ddf = "Kenward-Roger")) # significant main effect of test mod.

```


```{r mcont_contrast}
print(pairs(emmeans(mcont, ~ test_mod), simple = "test_mod", adjust = "holm")) # no significant differences between test modalities. 
...
Random effects are roughly normally distributed. No heteroskedasticity is identified and residuals appear normally distributed, except for quadriceps:exp. No influential observation is detected (cooks distance < 1).
...
```
Random effects are roughly normally distributed. No heteroskedasticity is identified and residuals are normally distributed---except for forearm:exp and quad:exp where a slight positive skew is observed. All cooks distance < 1: no influential observation is detected.
...
Random effects are roughly normally distributed. No heteroskedasticity is identified residuals and appear negatively skewed for all site:condition levels. No influential observation is detected. 
...
We choose the model including skin temperature.
...
Random effects are roughly normally distributed. No heteroskedasticity is identified and residuals follow a normal distribution with fat tails. No influential observation is detected (cooks distance < 1).
...
### Plot  models condition:site

```{r fun_plot_eih_s}
my_theme_eih_s <- function(base_size = 11, 
                     base_family = 'Roboto',
                     base_line_size = base_size / 22,
                     base_rect_size = base_size / 22,
                     md = T, col_x = "black", col_y = "black") {
  theme(
    plot.margin = margin(0.5, 0.5, 2, 0.5),
    axis.line.y = element_line(colour = "black"), 
    axis.line.x = element_line(colour = "black"), 
    panel.background = element_blank(), 
    plot.title = element_text(hjust = 0, size = base_size, face = "bold", colour = "black"), 
...
#  y_axis = T
  ...
  if(y_axis == T){
    y_title_points <-  y_title_means <- "Absolute change (z-scores)"
   ...
   y_min <- floor(min(d.all$tm) * 1000) / 1000
   y_max <- ceiling(max(d.all$tm) * 1000) / 1000
   y_breaks <- pretty(c(y_min, y_max))
   y_lim <- c(min(y_breaks), max(y_breaks))
  
 
  
  pos4 <- position_dodge2(width = 0.2)
  p_points_cont <- d.all |> 
    filter(condition == "Control") |>
    ggplot()+
    geom_hline(yintercept = 0, col = "black", linetype = "dashed") +
    geom_point(aes(y = tm, x = site, col = condition) , position = pos4, alpha = 0.7, size = 1,  show.legend = F) +
    ...
    my_theme_eih_s() +
     coord_cartesian(ylim = y_lim, expand = F, xlim = c(0.5, 2.5)) +
    scale_y_continuous( limits = y_lim ,  breaks = y_breaks,labels = function(x) format(round(x, 2), nsmall = 1), expand = c(0,0)) +
    labs(title = t,
  ...
  p_points_exp <- d.all |> 
    ...
    theme(
      ...
      y_min <- floor(min(d.sum$cil) * 1000) / 1000
   y_max <- ceiling(max(d.sum$cih) * 1000) / 1000
   y_breaks <- pretty(c(y_min*1.3, y_max*1.6))
   y_lim <- c(min(y_breaks), max(y_breaks))
...
#plot_eih_s(dm.ppt, t = "Pressure pain threshold", u = "Log", mod = m.0.ppt, x = F, y_axis = F)[[1]]
pppt.means <- plot_eih_s(dm.ppt, t = "Pressure pain threshold", u = "Log", mod = m.0.ppt, x = F, y_axis = F)[[3]]+
   showSignificance( 2.2, c(-0.16, 0.18), "*", width = -0.05) +
  showSignificance( c(1.05,2.1), 0.35, "*", width = -0.05, segmentParams = list(col = "#ED0000FF"), textParams = list(col = "#ED0000FF")) + 
  showSignificance( c(0.9,1.95), -0.32, "*", width = 0.05, segmentParams = list(col = "#00468BFF"), textParams = list(col = "#00468BFF"))  + theme(legend.position = "none")
  

pcdt.points <-  plot_eih_s(dm.cdt, t = "Cold detection threshold", u = "Log", mod = m.0.cdt, x = T)[[2]]
#plot_eih_s(dm.cdt, t = "Cold detection threshold", u = "Log", mod = m.0.cdt,x = T, y_axis = F)[[1]]

pcdt.means <- plot_eih_s(dm.cdt, t = "Cold detection threshold", u = "Log", mod = m.0.cdt,x = T, y_axis = F)[[3]] + 
  showSignificance( 2.2, c(-0.27, 0.11), "*", width = -0.05) +
  showSignificance( c(1.05,2.1), -0.5, "*", width = 0.05, segmentParams = list(col = "#ED0000FF"), textParams = list(col = "#ED0000FF"))+ theme(legend.position = "none")

phpt.points <-  plot_eih_s(dm.hpt, t = "Heat pain threshold", u = "°C", mod = m.1.hpt)[[2]]
phpt.means <- plot_eih_s(dm.hpt, t = "Heat pain threshold", u = "°C", mod = m.1.hpt, y_axis = F)[[3]]+ theme(legend.position = "none")
...
ggsave("/Users/vladimiraron/Library/CloudStorage/OneDrive-UCL/PhD/Projects/WP1/writing/figures_wp1/fig4_mcs.png", plot = p_mcs , dpi = 600, height = 25, width = 19, units = "cm")

ggsave("/Users/vladimiraron/Library/CloudStorage/OneDrive-UCL/PhD/Projects/WP1/writing/figures_wp1/fig4.tiff", plot = p_mcs , dpi = 600, height = 25, width = 19, units = "cm")
ggsave("/Users/vladimiraron/Library/CloudStorage/OneDrive-UCL/PhD/Projects/WP1/writing/wp1_manuscript_20251122_ejp/wp1_submission_EJP/fig4.png", plot = p_mcs , dpi = 600, height = 25, width = 19, units = "cm")
p_mcs
```

## Models value ~ timing * condition | test modality:site

The effects of timing  condition on sensory test values are investigated in each test modality:site where a significant effect of condition was found, and in ADT, using linear mixed model with a random intercept on the participant. 
...
d.mod.tms <- function(test_mod, site){
  ...
  # output: filtered data set formatted to be used by a lme : test_value ~ condition*timing + (1|ID)
...
dm.ppt.quad <- d.mod.tms("ppt","quad")  |>
  rowwise() |> 
  mutate(test_value = logt(test_value)) #log transormation
...
Random effects are normally distributed. No heteroskedasticity is identified and residuals appear normally distributed, a slight deviation is observed for post exp (negative skew). No influential observation is detected. Cooks distance < 1 suggest little influence of deviated data point. We keep the model. Data was log10 transformed.

```{r ppt_quad_estim}

print(summary(m.0.pptquad, ddf = "Kenward-Roger"))
...
m.0.cdtquad <- lmer(test_value ~  temp + timing*condition + (1|ID), data = dm.cdt.quad)
check_mod_tms(m.0.cdtquad, dm.cdt.quad)

```

An influential observation is detected and removed.

```{r cdt_quad_noout}
dm.cdt.quad.noout <- cooks_outliers( m.0.cdtquad, dm.cdt.quad)

m.0.cdtquad.noout <- lmer(test_value  ~ temp +  timing*condition + (1|ID), data = dm.cdt.quad.noout)
...
Random effects are not fully normal (positive skew). No heteroskedasticity is identified and residuals are normally distributed. No influential observation is detected. Data was log10 transformed according to standard procedure. 


```{r cdt_quad_temp}
m.0.cdtquad.noout.notemp <- lmer(test_value  ~  timing*condition + (1|ID), data = dm.cdt.quad.noout)

print(anova(m.0.cdtquad.noout,m.0.cdtquad.noout.notemp))
```

Models are compared to determine the inclusion of skin temperature. Significant LRT suggest to keep skin temperature in the model.

```{r cdt_quad_estim}
print(summary(m.0.cdtquad.noout, ddf = "Kenward-Roger"))
...
Random effects are not normally distributed. No heteroskedasticity is identified and residuals appear normal with fat tails. Extreme values appear despite cooks distance < 1 suggesting little impact on models output. 
...
No impact of log transformation is observed on the distribution of random effects. We decide to estimate the model with original units. 

```{r adt_estim}

print(summary(m.0.adt, ddf = "Kenward-Roger"))
...
t_models_pp <- an_mpp |> 
  ...
  add_header_row(values = c( "Test modalities", "Site","Effect","SS", "MS", "Df num", "Df den", "F value", "p", "Effect size" ), colwidths = c(1,1,1,1,1,1,1,1,1,2)
   ...
   hline(i = c(3, 7), border = dash_border)  |> 
    merge_v(j = c(1,2,3), part = "body") |> 
  merge_v(part = "header") |>
  valign(part = "body", valign = "top") |> 
    bold(part = "header")  |> 
...
my_theme_prepost <- function(base_size = 11, 
                     base_family = 'Roboto',
                     base_line_size = base_size / 22,
                     base_rect_size = base_size / 22,
                     md = T, col_x = "black", col_y = "black") {
...
# v = "PPT_Quadriceps"
 ...
 # v = "ADT_No site"
  ...
  y_min <- floor(min(d.sum$cil) * 1000) / 1000
   ...
   range = y_max - y_min
  perc_range = 0.2*range
   
   
   y_breaks <- pretty(c(y_min-perc_range, y_max+2*perc_range))
   y_lim <- c(min(y_breaks), max(y_breaks))
  ...
  p_points_cont <- d.all |> 
    filter(condition == "Control") |>
    ggplot()+
    geom_point(aes(y = tm, x = timing, col = condition) , position = pos4, alpha = 0.7, size = 1,  show.legend = F) +
  ...
  ls1 <- list(
    ...
    dsum = d.sum,
    pmeans = p_means,
    ppoins_cont = p_points_cont,
    ppoints_exp = p_points_exp
  )
...
ppptq <- plot_prepost_an(dm.ppt.quad, t = "Pressure pain threshold", u = "Log", df_an = an_mpp, v = "PPT_Quadriceps", y = "test_value", x = F)[[1]] + showSignificance( c(1.05,2.1),2.87 , "*", width = -0.005, segmentParams = list(col = "#ED0000FF"), textParams = list(col = "#ED0000FF")) +
  showSignificance( c(0.9,1.95), 2.75, "*", width = 0.005, segmentParams = list(col = "#00468BFF"), textParams = list(col = "#00468BFF")) +
  showSignificance( 2.3, c(2.79,2.83) , "*", width = -0.05)  
...
pcdtq <- plot_prepost_an(dm.cdt.quad.noout, "Cold detection threshold", "Log", df_an = an_mpp, v = "CDT_Quadriceps", x = F)[[1]] 




# ADT
padt <- (plot_prepost_an(dm.adt, "Auditory detection threshold", "/1", df_an = an_mpp, v = "ADT_No site"))[[1]] + theme(legend.position = "bottom")
...
ggsave("/Users/vladimiraron/Library/CloudStorage/OneDrive-UCL/PhD/Projects/WP1/writing/figures_wp1/fig5_mpp.png", plot = p_mpp, dpi = 600, height = 19, width = 19, units = "cm")

ggsave("/Users/vladimiraron/Library/CloudStorage/OneDrive-UCL/PhD/Projects/WP1/writing/figures_wp1/fig5.tiff", plot = p_mpp, dpi = 600, height = 19, width = 19, units = "cm")

ggsave("/Users/vladimiraron/Library/CloudStorage/OneDrive-UCL/PhD/Projects/WP1/writing/wp1_manuscript_20251122_ejp/wp1_submission_EJP/fig5.png", plot = p_mpp, dpi = 600, height = 19, width = 19, units = "cm")
...
Results from models are summarized, along with summary statistics in a table. Data is shown in natural units. Two other tables are made to display results in log transformed CDT and PPT and in standardized values for each test_modality:site.
...
dt_mods <- data_long |> 
  ...
  summarise(m = mean(ind_m, na.rm = T), 
            sd = sd(ind_m, na.rm = T)) |> 
  mutate(m = if_else(test_mod %in% c("ppt", "pp"), specify_decimal(m,0), specify_decimal(m,2))) |> 
  mutate(sd = if_else(test_mod %in% c("ppt", "pp"), specify_decimal(sd,0), specify_decimal(sd,2))) |> 
  mutate(m = if_else(test_mod %in% c("hpt", "cdt"), specify_decimal(as.numeric(m), 1), m)) |> 
  mutate(sd = if_else(test_mod %in% c("hpt", "cdt"), specify_decimal(as.numeric(sd), 1), sd)) |> 
  mutate(msd = glue("{m} ± {sd}")) |> 
  select(-m, -sd) |> 
...
p_int_mcs <- an_mcs |> 
  ...
  arrange(test_mod) |>
  filter(term == "condition:site") |> 
  mutate(across(c(DenDF), round, 0)) |> 
    mutate(across(c(statistic), round, 2)) |> 
...
t3 <- dt_mods |> 
  select(test_mod,site, pre_cont, post_cont, abc_cont, pre_exp, post_exp, abc_exp, fpp, ppp,  baseline, eih, ffull, pfull) |> 
   mutate(
         test_mod = factor(test_mod, levels = c("PPT", "PP", "HPT", "CDT", "ADT"), labels = c("PPT (kPa)", "PP (100 - 0)", "HPT (°C)", "CDT (ΔT, °C)", "ADT (/1)"))) |> 
  arrange(test_mod) |> 
  flextable() |> 
  set_header_labels(
     test_mod = "Test modality",site = "Site", pre_cont = "Pre", post_cont = "Post" , abc_cont = "Diff~post-pre~", pre_exp = "Pre", post_exp = "Post" , abc_exp = "Diff~post-pre~", fpp = "F~df1,df2~", ppp = "p", baseline = "Baseline~diff~", eih = "EIH", ffull = "F~df1,df2~", pfull = "p"
  ) |> 
  add_header_row(values = c(
     "Test modality","Site", "Control", "Exercise", "Site x Condition interaction", "Baseline~diff~", "EIH", "Test modality x Site interaction"
  ), colwidths = c(1,1,3,3,2,1,1,2)) |> 
  colformat_md(part = "all", j = 1:14) |> 
  merge_v(part = "header") |> 
  bold(part = "header") |> 
  align(part = "header", align = "center") |> 
  align(part = "body", j = 1:2, align = "left") |> 
  align(part = "body", j = 3:14, align = "right") |> 
  ...
  set_table_properties(layout = "autofit") |> 
  add_footer_lines(values = c(
"Data from 38 participants, reported as mean ± SD. Units or maximum scores are indicated in parentheses. \n # F statistics and p value reported from timing x condition interaction. \n Abbreviations: PPT, Pressure pain threshold; PP, Mechanical pinprick rating; HPT, Heat pain threshold; CDT, Cold detection threshold; Diff post - pre, Difference of post and pre values in each condition; kPa, kilopascal; °C, Celsius degree; ΔT, difference from baseline temperature (32°C); df, degrees of freedom.") )
...
PPT and CDT values were log10 transformed, values are reported in the table below.
...
Models on EIH and absolute change used standardized values to take into account variability in test modalities and site. PPT and CDT were log10 transformed prior to this procedure.
...
std_dat <- data_long |> 
  ...
  rowwise() |> 
    mutate(test_value = if_else(test_mod %in% c("ppt", "cdt"), logt(test_value), test_value)) |> 
  group_by(test_mod, site) |> 
  mutate(test_value = scale(test_value)) #standardization (i.e., z-scoring)

std_dat |> 
  group_by(test_mod, site) |> 
  summarise(
    m = round(mean(test_value, na.rm = T),3),
    sd = round(sd(test_value, na.rm = T),3)
  )
```

A third table of summary statistics is  computed presenting pre, post, and absolute change values in each condition and timing levels after standardization. 
